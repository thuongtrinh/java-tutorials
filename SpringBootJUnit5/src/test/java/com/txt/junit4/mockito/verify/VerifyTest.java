package com.txt.junit4.mockito.verify;
...
public class VerifyTest {
	...
	public void zeroTimeTest() {
		...
		Mockito.verify(mockedList, Mockito.never()).add("com.txt");
		Mockito.verify(mockedList, Mockito.times(0)).add("com.txt");
	}
	...
	public void oneTimeTest() {
		...
		Mockito.verify(mockedList).add("com.txt"); // Default one time
		Mockito.verify(mockedList, Mockito.times(1)).add("com.txt");
	}
	...
	public void atLeastTest() {
		...
		Mockito.verify(mockedList, Mockito.atLeast(3)).add(Mockito.anyString());
	}
	...
	public void verifyNoMoreInteractionsTest() {
		...
		// Uncomment this code will make the test case failed because it is
		// unverified interaction mockOne.get(0);

		// Ordinary verification
		Mockito.verify(mockOne).add("one");

		// Checks if any of given mocks has any unverified interaction. You can use this
		// method after you verified your mocks - to make sure that nothing else
		// was invoked on your mocks.
